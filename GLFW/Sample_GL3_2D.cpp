...
void IncreaseLevel(){
	if(fall_dy<=3){
	cout << "Current Level is: " << 3 + (fall_dy-1)*5 <<endl;
		fall_dy+=0.2;
	}
	else{
		cout << "Maximum level 13 reached!"<< endl;
	}
}
void DecreaseLevel(){
	if(fall_dy>=0.6){
	cout << "Current Level is: " << 3 + (fall_dy-1)*5 <<endl;
		...
		cout << "Minimum level 1 reached!"<< endl;
...
void recoilCannon(){
	if(shot){
		double time_diff = glfwGetTime() - cannon_last_time;
		if(time_diff <= 1){
			button["A"]=false;
			button["D"]=false;
			cannon_color_shot = sin(time_diff*M_PI);
			cannon.pos_x = cannon_back_x - cannon.width/2*sin(time_diff*M_PI)*cos(cannon.rotation*M_PI/180);
			cannon.pos_y = cannon_back_y - cannon.width/2*sin(time_diff*M_PI)*sin(cannon.rotation*M_PI/180);
			//cout << sin(time_diff*M_PI)<<endl;
		}
		else{
			cannon.pos_y = cannon_back_y;
			cannon.pos_x = cannon_back_x;
			shot=false;
		}
	}
}
...
void createGameOverScreen(){
	...
	screen.width=800;
	screen.height=800;
	float body_width=screen.width;
	float body_height=screen.height;
	...
	COLOR colorA;
		colorA.r=(float)255/255,
		colorA.b=(float)182/255,
		colorA.g=(float)193/255;
	static const GLfloat color_buffer_data [] = {
		colorA.r,colorA.g,colorA.b,//1
		colorA.r,colorA.g,colorA.b,//2
		colorA.r,colorA.g,colorA.b,//3
	...
	screen.object = create3DObject(GL_TRIANGLES, 12, vertex_buffer_data, color_buffer_data, GL_FILL);;
}
...
void draw ()
{
	...
	if(game_over){
		...
		glm::mat4 translateScreen = glm::translate (glm::vec3(screen.pos_x, screen.pos_y, 0));        // glTranslatef
		glm::mat4 rotateScreen = glm::rotate((float)(screen.rotation*M_PI/180.0f), glm::vec3(0,0,1)); // rotate about vector (-1,1,1)
		Matrices.model *= (translateScreen * rotateScreen);
		MVP = VP * Matrices.model;
		glUniformMatrix4fv(Matrices.MatrixID, 1, GL_FALSE, &MVP[0][0]);

		// draw3DObject draws the VAO given to it using current MVP matrix
		draw3DObject(screen.object);
	}
	Matrices.model = glm::mat4(1.0f);
...
void initGL (GLFWwindow* window, int width, int height){
	...
	createBucket (0);
	createBucket (1);
	createMirror ( 0,0,50,60);
	createMirror ( 200,-200,150,45);
	createMirror ( 200,200,150,135);
	createBrickArea ();
	...
	glClearColor (135.0f/255, 206.0f/255, 250.0f/255, 0.0f); // R, G, B, A
	glClearDepth (1.0f);
...
int colliBuckBrick(Sprite buck,list<Sprite>::iterator it){
	if( it->pos_y + it->height <= buck.pos_y + buck.height && it->pos_y>= buck.pos_y + buck.height/5 ){
		...
		if( it->pos_x + it->width <= buck.pos_x+buck.width && it->pos_x >= buck.pos_x ){
			if(it->color==2){
				return -1; // the brick is black
			}
			else if(it->color==buck.color){
				return 2; // the brick matches the color of the bucket
			}
			else if(it->color!=buck.color){
				 // the brick matches the color of the bucket
				return 1;
			}
		}
	}
...
bool colliLaserMirror(vector<Sprite>::iterator it, list<Sprite>::iterator laser)
{
	for(int i=-it->length/2+1 + it->height;i<it->length/2;i+=it->height){
		...
		if(cy <=it->height  && (theta-fi>0 || theta-fi<-180)){
			return true;
		}
...
int main (int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if ((current_time - last_update_time) >= 1) { // atleast 0.5s elapsed since last frame
			...
			if(game_over){
				cout << "game will quit in "<<(int) gameOverTimer<<"seconds"<<endl;
			}
			gameOverTimer-=1;
			if(game_over && gameOverTimer<0){
				cout << "Game quit automatically due to no response."<<endl;
				break;
			}
